...
function question(prompt, rl, scriptedState) {
  return new Promise((resolve) => {
    if (Array.isArray(scriptedState.answers)) {
      process.stdout.write(prompt);
      const answer = scriptedState.answers[scriptedState.index++] || '';
...
async function interactiveSetup() {
  ...
  const scriptedState = { answers: scriptedAnswers, index: 0 };
  
  const answers = {};

  try {
    // Question 1: Collections folder
    let collectionsFolder = await question('📂 Where are your collection files located? (default: ./collections): ', rl, scriptedState);
    ...
    if (!fs.existsSync(answers.collectionsFolder)) {
      ...
      const create = await question('Create it? (yes/no): ', rl, scriptedState);
    ...
    const useFolder = await question('Auto-discover collections in folder? (yes/no, default: yes): ', rl, scriptedState);
    if (useFolder.toLowerCase() !== 'no' && useFolder.toLowerCase() !== 'n') {
      ...
      let count = 1;
      while (addMore) {
        const collPath = await question(`Collection ${count} path (or leave empty to skip): `, rl, scriptedState);
        if (collPath.trim()) {
          answers.collections.push({ collection: collPath.trim() });
          count++;
          const continueAdding = await question('Add another collection? (yes/no): ', rl, scriptedState);
    ...
    const parallelAnswer = await question('Run collections in parallel? (yes/no, default: yes): ', rl, scriptedState);
    answers.parallel = parallelAnswer.toLowerCase() !== 'no' && parallelAnswer.toLowerCase() !== 'n';

    // Question 4: Continue on error behavior
    const continueOnErrorAnswer = await question('Continue running other collections when one fails? (yes/no, default: yes): ', rl, scriptedState);
    answers.continueOnError = continueOnErrorAnswer.toLowerCase() !== 'no' && continueOnErrorAnswer.toLowerCase() !== 'n';

    // Question 5: Environment support
    const useEnv = await question('Use environment files? (yes/no, default: no): ', rl, scriptedState);
    if (useEnv.toLowerCase() === 'yes' || useEnv.toLowerCase() === 'y') {
      answers.environmentsFolder = await question('Environments folder (default: ./environments): ', rl, scriptedState);
      answers.environmentsFolder = answers.environmentsFolder.trim() || './environments';
    }

    // Question 6: Reporter configuration
    console.log('\n📊 Reporter options:');
    console.log('  1. cli (terminal output)');
    console.log('  2. json (results in JSON)');
    console.log('  3. html (HTML report)');
    const reporters = await question('Choose reporters (comma-separated, default: cli,json): ', rl, scriptedState);
    answers.reporters = normalizeReporters(reporters, true);

    // Question 7: Output directory
    const outputDir = await question('Results output directory (default: ./results): ', rl, scriptedState);
...
async function generateConfigFromAnswers(answers) {
  const config = {
    ...
    continueOnError: typeof answers.continueOnError === 'boolean' ? answers.continueOnError : true,
